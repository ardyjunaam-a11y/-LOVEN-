...
create policy "produk_admin_all" on produk
  for all using (auth.jwt() ->> 'email' = 'email-admin-anda@gmail.com')
  with check (auth.jwt() ->> 'email' = 'email-admin-anda@gmail.com');
...
> ⚠️ Ganti `email-admin-anda@gmail.com` di **setiap** query di atas dengan email Google admin yang sama seperti di `admin.html` (variabel `ALLOWED_ADMIN_EMAIL`).
...
## 6. Isi konfigurasi di kedua file HTML

Di `index.html` dan `admin.html`, ganti:
```js
const SUPABASE_URL = "URL_SUPABASE";
const SUPABASE_ANON_KEY = "ANON_KEY_SUPABASE";
```
dengan URL & anon key project Supabase kamu (Settings → API).

Di `admin.html`, ganti juga:
```js
const ALLOWED_ADMIN_EMAIL = "email-admin-anda@gmail.com";
```
...
- **Customer** buka `index.html` → login Google atau daftar email/password → belanja → checkout → isi data → scan QRIS / lihat info transfer → upload bukti bayar → pesanan masuk dengan status **"Menunggu Konfirmasi"**.
- **Admin** buka `admin.html` → login Google (khusus email admin) → tab **Pesanan** bisa lihat bukti bayar & ubah status (Diproses / Selesai / Dibatalkan) → tab **Pengaturan** untuk ganti QRIS & info rekening → tab **Produk** untuk kelola menu kue.
